...
class NewVisitorTest(unittest.TestCase):
	...
	# here is the first test (for free) 
        ...
        #1 testing for a page title on the home page
        self.assertIn('Quiz 3 Test Web Site',self.browser.title)

        #2 testing for an h1 type heading on the home page
        h=self.browser.find_element_by_tag_name('h1')

        #3 testing for an image of your choice
        m=self.browser.find_element_by_tag_name('img')
        self.assertIn('running.jpg',m.get_attribute('src'))

        #4 testing for a clickable area that leadsto "newpage.html"
        a=self.browser.find_element_by_id('homepage')
        a.click()

        #5 testing for the existence of "newpage.html"
        self.assertIn('newpage',self.browser.title)

        #6 testing for a header on the new page
        h=self.browser.find_element_by_tag_name('h1')
